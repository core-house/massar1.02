...
>
    ...
    <div class="async-select-input-wrapper position-relative w-100">
        ...
        <div
            ...
            x-cloak
            class="async-select-dropdown position-absolute w-100 mt-1 rounded border shadow-lg bg-white"
            style="z-index: 1050; top: 100%; left: 0; right: 0; overflow: hidden;"
            @mousedown.prevent
        >
                ...
                @if ($this->hasGroups)
                    ...
                    @foreach ($this->groupedOptions as $groupName => $groupOptions)
                        @if ($groupName !== '_flat' && $groupName !== '_ungrouped')
                            <div class="list-group-item list-group-item-secondary small fw-bold py-2 px-3">
                                {{ $groupName }}
                            </div>
                        @endif
