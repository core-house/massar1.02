...
use App\Models\{OperHead, OperationItems, JournalHead, AccHead, Employee, Item, JournalDetail};
...
class POSController extends Controller
{
    ...
    public function reports()
    {
        ...
        // إحصائيات اليوم للمستخدم الحالي
        $todayStats = [
            ...
            'transactions_count' => OperHead::where('pro_type', 10)
                ...
                ->whereDate('created_at', today())
                ->count(),
            'items_sold' => OperationItems::whereHas('operhead', function($query) {
        ...
        // تحليل طرق الدفع لليوم
        $paymentAnalysis = [
            'cash_total' => OperHead::where('pro_type', 10)
                ->where('user', auth()->id())
                ->whereDate('created_at', today())
                ->whereNull('paid_from_client') // نقدي كامل
                ->sum('fat_net') ?? 0,
            'partial_payment' => OperHead::where('pro_type', 10)
                ->where('user', auth()->id())
                ->whereDate('created_at', today())
                ->whereNotNull('paid_from_client')
                ->where('paid_from_client', '>', 0)
                ->sum('paid_from_client') ?? 0,
        ];

        // إحصائيات إضافية
        $additionalStats = [
            'avg_transaction_value' => $todayStats['transactions_count'] > 0 
                ? round($todayStats['total_sales'] / $todayStats['transactions_count'], 2) 
                : 0,
            'last_transaction_time' => OperHead::where('pro_type', 10)
                ->where('user', auth()->id())
                ->whereDate('created_at', today())
                ->latest()
                ->value('created_at'),
        ...
        return view('pos::reports', compact('todayStats', 'paymentAnalysis', 'additionalStats'));
    }
}