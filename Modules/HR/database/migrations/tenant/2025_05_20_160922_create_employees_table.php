...
class CreateEmployeesTable extends Migration
{
    public function up()
    {
        Schema::create('employees', function (Blueprint $table) {
            ...
            $table->enum('marital_status', ['متزوج', 'غير متزوج', 'مطلق', 'أرمل'])->nullable();
            $table->enum('education', ['دبلوم', 'بكالوريوس', 'ماجستير', 'دكتوراه'])->nullable();
            ...
            $table->enum('job_level', ['مبتدئ', 'متوسط', 'محترف'])->nullable();
            $table->decimal('salary', 10, 2)->nullable();
            $table->integer('finger_print_id')->nullable()->unique();
            $table->string('finger_print_name')->nullable()->unique();
            $table->enum('salary_type', ['ساعات عمل فقط', 'ساعات عمل و إضافي يومى', 'ساعات عمل و إضافي للمده', 'حضور فقط', 'إنتاج فقط'])->nullable();
