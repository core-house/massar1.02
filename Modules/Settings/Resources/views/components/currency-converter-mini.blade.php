...
@php
    ...
    // Resolve To Currency Code if ID is passed
    if ($selectedCurrency) {
        if (is_numeric($selectedCurrency)) {
            $currencyModel = \Modules\Settings\Models\Currency::find($selectedCurrency);
            $finalToCurrency = $currencyModel ? $currencyModel->code : $toCurrency;
        } else {
            $finalToCurrency = $selectedCurrency;
        }
    }

    // Resolve From Currency Code (Base) if not provided
    if (!$fromCurrency) {
        $defaultCurrency = getDefaultCurrency();
        $fromCurrency = $defaultCurrency ? $defaultCurrency->code : 'EGP';
    }
@endphp
...
{{ $attributes->merge(['class' => 'currency-converter-mini ' . $class]) }}>
    ...
    <div class="input-group input-group-sm flex-nowrap align-items-center bg-secondary rounded-pill p-1 border" style="max-width: 350px;">
            ...
            <select x-model="toCurrency" 
                @change="
                    ...
                    if (selected) {
                        ...
                        $dispatch('currency-id-changed', { id: selected.id });
                    }
                "
