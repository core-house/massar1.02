        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h2>{{ __('crm::crm.edit_category') }}</h2>
                </div>
                <div class="card-body">
                    <form action="{{ route('tasks.type-categories.update', $category->id) }}" method="POST" onsubmit="disableButton()">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row">
                            <div class="mb-3 col-lg-6">
                                ...
                                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name"
                                    placeholder="{{ __('crm::crm.enter_the_name') }}" value="{{ old('name', $category->name) }}">
                                @error('name')
                            ...
                            <div class="mb-3 col-lg-6">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                                    rows="3" placeholder="{{ __('crm::crm.enter_category_description') }}">{{ old('description', $category->description) }}</textarea>
                                @error('description')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
