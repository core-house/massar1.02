...
@endsection
@section('content')
    {{-- Debug: {{ dd($invoice->pro_serial) }} --}}
    <div class="min-h-screen bg-gray-50" id="manufacturing-form-container">
        <!-- Header -->
        <header class="bg-white border-bottom sticky-top px-4 py-2 shadow-sm">
            ...
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    ...
                    @if(auth()->user()->branches->count() > 1)
                        <select id="branch-id" name="branch_id" class="form-select border-0 bg-light" style="width: 150px; height: 28px; font-size: 0.8rem; padding: 0.2rem 0.5rem;">
                            @foreach(auth()->user()->branches as $branch)
                                <option value="{{ $branch->id }}" {{ $branch->id == auth()->user()->current_branch_id ? 'selected' : '' }}>
                                    {{ $branch->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <!-- Employee -->
                    <select id="employee-select-visible" class="form-select border-0 bg-light" style="width: 140px; height: 28px; font-size: 0.8rem; padding: 0.2rem 0.5rem;" title="{{ __('manufacturing::manufacturing.employee') }}">
                        @foreach ($employees as $employee)
                            <option value="{{ $employee->id }}" {{ ($invoice->emp_id ?? '') == $employee->id ? 'selected' : ($loop->first ? 'selected' : '') }}>
                                {{ $employee->aname }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    {{-- <select id="salaries-account-select" class="form-select border-0 bg-light" style="width: 160px; height: 28px; font-size: 0.8rem; padding: 0.2rem 0.5rem;" title="{{ __('Salaries and Wages Payable') }}">
                        ...
                        @foreach ($accounts ?? [] as $account)
                            <option value="{{ $account->id }}" {{ ($invoice->salaries_account_id ?? '') == $account->id ? 'selected' : '' }}>{{ $account->aname }}</option>
                        @endforeach
                    </select> --}}

                    <!-- Date -->
                    <input type="date" id="display-invoice-date" value="{{ $invoice->pro_date }}"
                           class="form-control border-0 bg-light" style="width: 145px; height: 28px; font-size: 0.8rem; padding: 0.2rem 0.5rem;"
                           title="{{ __('manufacturing::manufacturing.date') }}"
                           {{ setting('allow_edit_manufacturing_date', true) ? '' : 'readonly' }}>

                    <!-- Invoice Number -->
                    <input type="text" id="display-invoice-number" value="{{ $invoice->pro_id }}" readonly
                    ...
                    <!-- Batch Number -->
                    <input type="text" id="display-patch-number" placeholder="{{ __('manufacturing::manufacturing.batch number') }}" value="{{ $invoice->pro_serial ?? '' }}"
                           class="form-control border-0 bg-light" style="width: 120px; height: 28px; font-size: 0.8rem; padding: 0.2rem 0.5rem;"
                ...
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    ...
                    <button type="button" id="btn-load-template"
                        class="btn btn-outline-secondary btn-sm"
                    ...
                    <button type="button" id="btn-distribute-costs"
                        ...
                        <span class="d-none d-lg-inline">{{ __('manufacturing::manufacturing.distribute costs by percentage') }}</span>
                    </button>
        ...
        <main class="w-full grid grid-cols-12 gap-6">
                ...
                <section class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="flex gap-3 mb-6">
                        ...
                        <select id="product-account"
                            ...
                            @foreach ($accounts as $account)
                                <option value="{{ $account->id }}" {{ $invoice->acc1 == $account->id ? 'selected' : '' }}>
                            ...
                            <div class="flex gap-3 mb-6">
                                ...
                                <select id="raw-material-account"
                                    ...
                                    @foreach ($rawMaterialAccounts as $account)
                                        <option value="{{ $account->id }}" {{ $invoice->acc2 == $account->id ? 'selected' : '' }}>
                                            {{ $account->aname }}</option>
                                    @endforeach
                                </select>
            ...
            <div class="col-span-12 lg:col-span-4 px-4" id="manufacturing-sidebar">
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        <div id="variance-warning"
                                            ...
                                            <div class="flex items-start gap-2">
                                                <svg class="h-5 w-5 text-red-500 flex-shrink-0 mt-0.5" fill="currentColor"
                                                    viewBox="0 0 20 20">
                                                    <path fill-rule="evenodd"
                            ...
                            <div class="grid grid-cols-2 gap-4 text-xs">
                                ...
                                <div class="space-y-2 border-e border-gray-200 pe-4">
                                    ...
                                    <div class="flex justify-between items-center py-1.5">
                                        ...
                                        <span class="font-bold text-blue-600 text-end" id="selected-item-available">-</span>
                                    </div>
                                    <div class="flex justify-between items-center py-1.5">
        ...
        <form id="manufacturing-form" method="POST" action="{{ route('manufacturing.update', $invoice->id) }}">
            @csrf
            @method('PUT')
            <!-- Data arrays -->
            <div id="form-products-container" class="hidden"></div>
            <div id="form-raw-materials-container" class="hidden"></div>
            <div id="form-expenses-container" class="hidden"></div>
            ...
            <!-- Invoice details -->
            <input type="hidden" name="pro_id" id="pro-id" value="{{ $invoice->pro_id }}">
            <input type="hidden" name="pro_date" id="invoice-date" value="{{ $invoice->pro_date }}">
            <input type="hidden" name="branch_id" id="form-branch-id" value="{{ $invoice->branch_id }}">

            <!-- Accounts -->
            <input type="hidden" name="acc1" id="product-account-input"
                value="{{ $invoice->acc1 ?? $accounts->first()->id ?? '' }}">
            <input type="hidden" name="acc2" id="raw-account-input"
                value="{{ $invoice->acc2 ?? $rawMaterialAccounts->first()->id ?? '' }}">
            <input type="hidden" name="emp_id" id="employee-id"
                value="{{ $invoice->emp_id ?? $employees->first()->id ?? '' }}">
            <input type="hidden" name="operating_account" id="operating-account"
                value="{{ $invoice->acc3 ?? 73 }}">

            <!-- Optional fields -->
            <input type="hidden" name="info" id="description" value="">
            <input type="hidden" name="pro_serial" id="patch-number" value="{{ $invoice->pro_serial ?? '' }}">
            <input type="hidden" name="order_id" id="order-id" value="">
            <input type="hidden" name="stage_id" id="stage-id" value="">
            <input type="hidden" name="actual_time" id="form-expected-time" value="">
            <input type="hidden" name="template_name" id="form-template-name" value="">
        </form>
    </div>
