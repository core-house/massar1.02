            ...
            <div class="card-body">
                            ...
                            <select name="branchFilter" class="form-select">
                                ...
                                @foreach ($branches as $branch)
                                    <option value="{{ $branch->id }}"
                                        {{ request('branchFilter') == $branch->id ? 'selected' : '' }}>
                                        {{ $branch->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <tr class="text-center">
                                ...
                                <th style="width: 10%" class="sortable" data-field="pro_id">
                                    ...
                                    @if (request('sortField') === 'pro_id')
                                        <i
                                            class="fas fa-sort-{{ request('sortDirection') === 'asc' ? 'up' : 'down' }}"></i>
                                    @endif
                                </th>
                        ...
                        <tbody>
                            @forelse($invoices as $invoice)
                                <tr>
                                    ...
                                    <td class="text-center">{{ $invoice->acc1Head->aname ?? '-' }}</td>
                                    ...
                                    <td class="text-center">
                                        @if ($invoice->expected_time)
                                            <span
                                                class="badge bg-primary">{{ rtrim(rtrim($invoice->expected_time, '0'), ':') }}</span>
                                        @else
                                    ...
                                    <td class="text-center">
                                        @if ($invoice->details)
                                            <span class="badge bg-success">{{ $invoice->details }}</span>
                                        @else
                                            <span class="text-muted">-</span>
                                        @endif
                                    </td>
                                    <td class="text-center">
                                        ...
                                        <small
                                            class="text-muted d-block">{{ __('manufacturing::manufacturing.egp') }}</small>
                                    </td>
                                    <td class="text-center">
                                        <div class="btn-group btn-group-sm" role="group">
                                            @can('view Manufacturing Invoices')
                                                <a href="{{ route('manufacturing.show', $invoice->id) }}"
                                                    class="btn btn-info"
                                                    title="{{ __('manufacturing::manufacturing.view') }}">
                                            ...
                                            @can('delete Manufacturing Invoices')
                                                <button onclick="confirmDelete({{ $invoice->id }})" class="btn btn-danger"
                ...
                <div class="d-flex justify-content-between align-items-center mt-3">
                    <div class="text-muted">
                        {{ __('manufacturing::manufacturing.showing') }} {{ $invoices->firstItem() ?? 0 }}
                        {{ __('manufacturing::manufacturing.to') }}
                        {{ $invoices->lastItem() ?? 0 }}
                        {{ __('manufacturing::manufacturing.of') }} {{ $invoices->total() }}
                        {{ __('manufacturing::manufacturing.invoices') }}
                    </div>
                    <div>
                        {{ $invoices->links() }}
                    </div>
