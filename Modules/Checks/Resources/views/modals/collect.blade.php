<!-- Modal {{ __('Collect Check') }} -->
<div class="modal fade" id="collectModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title"><i class="fas fa-university"></i> {{ __('Collect Check') }}</h5>
            ...
            <form id="collectForm">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label fw-bold">{{ __('Choose account') }} <span class="text-danger">*</span></label>
                        <select name="bank_account_id" id="bank_account_id" class="form-select" required>
                            ...
                            @foreach($banks as $bank)
                                <option value="{{ $bank->id }}">
                                    ...
                                    ({{ __('Balance:') }} {{ number_format($bank->balance, 2) }})
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="alert alert-info">
                        ...
                        {{ __('Selected checks will be transferred to the chosen bank and accounting entry will be created') }}
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-check"></i> {{ __('Collect Now') }}
                    </button>
                </div>
            </form>
        </div>
...
function collectSelected() {
    ...
    $('#selectedChecksInfo').html(`
        <div class="alert alert-success">
            <strong>{{ __("Number of selected checks:") }}</strong> ${selected.length}
        </div>
    `);
