...
@section('content')
    @include('components.breadcrumb', [
        'title' => __('sops::sops.add_new'),
        'breadcrumb_items' => [
            ['label' => __('Dashboard'), 'url' => route('admin.dashboard')],
            ['label' => __('sops::sops.sops'), 'url' => route('sops.index')],
            ['label' => __('sops::sops.add_new')],
        ],
    ])
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                    <option value="draft" {{ old('status') == 'draft' ? 'selected' : '' }}>{{ __('sops::sops.draft') }}</option>
                                    <option value="active" {{ old('status') == 'active' ? 'selected' : '' }}>{{ __('sops::sops.active') }}</option>
                                    <option value="archived" {{ old('status') == 'archived' ? 'selected' : '' }}>{{ __('sops::sops.archived') }}</option>
                                </select>
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="text" name="version" class="form-control @error('version') is-invalid @enderror" value="{{ old('version', '1.0') }}">
