...
<tr class="{{ $rowClasses }}" data-level="{{ $level }}" data-account-id="{{ $account->id }}"
    data-parent-id="{{ $parentIdentifier }}" data-has-children="{{ $hasChildren ? 1 : 0 }}"
    data-balance="{{ $balanceValue }}" data-total-with-children="{{ $totalWithChildrenValue }}"
    @if ($section) data-section="{{ $section }}" @endif>
    <td class="align-middle">
        @if ($hasChildren)
            <button type="button" class="{{ $buttonClasses }}" data-account-id="{{ $account->id }}" aria-expanded="true"
