...
<div class="container-fluid">
    <div class="row mb-4">
        <div class="col-12">
            <h2><i class="fas fa-edit me-2"></i>تعديل التقرير: {{ $ncr->ncr_number }}</h2>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="batch_number" class="form-control" 
                                       value="{{ old('batch_number', $ncr->batch_number) }}" placeholder="اختياري">
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="source" class="form-select @error('source') is-invalid @enderror" required>
                                    ...
                                    <option value="receiving_inspection" {{ old('source', $ncr->source) == 'receiving_inspection' ? 'selected' : '' }}>فحص استلام</option>
                                    <option value="in_process" {{ old('source', $ncr->source) == 'in_process' ? 'selected' : '' }}>أثناء الإنتاج</option>
                                    <option value="final_inspection" {{ old('source', $ncr->source) == 'final_inspection' ? 'selected' : '' }}>فحص نهائي</option>
                                    <option value="customer_complaint" {{ old('source', $ncr->source) == 'customer_complaint' ? 'selected' : '' }}>شكوى عميل</option>
                                    <option value="internal_audit" {{ old('source', $ncr->source) == 'internal_audit' ? 'selected' : '' }}>تدقيق داخلي</option>
                                    <option value="supplier_notification" {{ old('source', $ncr->source) == 'supplier_notification' ? 'selected' : '' }}>إشعار مورد</option>
                                </select>
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <textarea name="problem_description" rows="4" 
                                          ...
                                          placeholder="اشرح المشكلة بالتفصيل..." required>{{ old('problem_description', $ncr->problem_description) }}</textarea>
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <textarea name="immediate_action" rows="3" class="form-control" 
                                          placeholder="ما هو الإجراء الفوري الذي تم اتخاذه؟">{{ old('immediate_action', $ncr->immediate_action) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-4">
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <select name="severity" class="form-select @error('severity') is-invalid @enderror" required>
                                <option value="minor" {{ old('severity', $ncr->severity) == 'minor' ? 'selected' : '' }}>ثانوي</option>
                                <option value="major" {{ old('severity', $ncr->severity) == 'major' ? 'selected' : '' }}>رئيسي</option>
                                <option value="critical" {{ old('severity', $ncr->severity) == 'critical' ? 'selected' : '' }}>حرج</option>
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="disposition" class="form-select">
                                ...
                                <option value="rework" {{ old('disposition', $ncr->disposition) == 'rework' ? 'selected' : '' }}>إعادة عمل</option>
                                <option value="scrap" {{ old('disposition', $ncr->disposition) == 'scrap' ? 'selected' : '' }}>إتلاف</option>
                                <option value="return_to_supplier" {{ old('disposition', $ncr->disposition) == 'return_to_supplier' ? 'selected' : '' }}>إرجاع للمورد</option>
                                <option value="use_as_is" {{ old('disposition', $ncr->disposition) == 'use_as_is' ? 'selected' : '' }}>استخدام كما هو</option>
                                <option value="repair" {{ old('disposition', $ncr->disposition) == 'repair' ? 'selected' : '' }}>إصلاح</option>
                                <option value="downgrade" {{ old('disposition', $ncr->disposition) == 'downgrade' ? 'selected' : '' }}>تخفيض الدرجة</option>
                            </select>
                        </div>
                ...
                <div class="d-grid gap-2">
                    <button type="submit" class="btn btn-warning btn-lg">
                        <i class="fas fa-save me-2"></i>تحديث التقرير
                    </button>
