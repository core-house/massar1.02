...
@section('content')
    @include('components.breadcrumb', [
        'title' => __('quality::quality.create new audit'),
        'breadcrumb_items' => [
            ...
            ['label' => __('quality::quality.create')],
        ],
    ])
                    ...
                    <form action="{{ route('quality.audits.store') }}" method="POST">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="audit_title" id="audit_title"
                                    ...
                                    value="{{ old('audit_title') }}" required>
                                @error('audit_title')
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="audit_type" id="audit_type"
                                    ...
                                    <option value="">-- {{ __('quality::quality.select audit type') }} --</option>
                                    <option value="internal" {{ old('audit_type') == 'internal' ? 'selected' : '' }}>{{ __('quality::quality.internal') }}</option>
                                    <option value="external" {{ old('audit_type') == 'external' ? 'selected' : '' }}>{{ __('quality::quality.external') }}</option>
                                    <option value="supplier" {{ old('audit_type') == 'supplier' ? 'selected' : '' }}>{{ __('quality::quality.supplier audit') }}</option>
                                    <option value="certification" {{ old('audit_type') == 'certification' ? 'selected' : '' }}>{{ __('quality::quality.certification') }}</option>
                                    <option value="customer" {{ old('audit_type') == 'customer' ? 'selected' : '' }}>{{ __('quality::quality.customer') }}</option>
                                </select>
                                @error('audit_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6 mb-3" id="external_fields" style="display: {{ in_array(old('audit_type'), ['external', 'certification']) ? 'block' : 'none' }}">
                                ...
                                <input type="text" name="external_auditor" id="external_auditor"
                                    ...
                                    value="{{ old('external_auditor') }}">
                                @error('external_auditor')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>

                            <div class="col-md-6 mb-3" id="external_org_field" style="display: {{ in_array(old('audit_type'), ['external', 'certification']) ? 'block' : 'none' }}">
                                ...
                                <input type="text" name="external_organization" id="external_organization"
                                    ...
                                    value="{{ old('external_organization') }}">
                            ...
                            <div class="col-12 mb-3">
                                ...
                                <textarea name="audit_objectives" id="audit_objectives" rows="3"
                                    class="form-control @error('audit_objectives') is-invalid @enderror">{{ old('audit_objectives') }}</textarea>
                        ...
                        <div class="mt-3">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save me-2"></i>{{ __('quality::quality.save audit') }}
                            </button>
