    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                <h4 class="page-title">{{ __('myresources.edit_category') }}</h4>
            </div>
        </div>
    </div>
                ...
                <div class="card-body">
                    <form action="{{ route('myresources.categories.update', $category) }}" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="name_ar" class="form-label">{{ __('myresources.arabic_name') }} <span class="text-danger">*</span></label>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="icon" id="icon" class="form-control @error('icon') is-invalid @enderror" value="{{ old('icon', $category->icon) }}" placeholder="fas fa-cogs">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="color" id="color" class="form-select @error('color') is-invalid @enderror">
                                    <option value="primary"   {{ old('color', $category->color) == 'primary'   ? 'selected' : '' }}>{{ __('common.color_blue') }}</option>
                                    <option value="success"   {{ old('color', $category->color) == 'success'   ? 'selected' : '' }}>{{ __('common.color_green') }}</option>
                                    <option value="info"      {{ old('color', $category->color) == 'info'      ? 'selected' : '' }}>{{ __('common.color_light') }}</option>
                                    <option value="warning"   {{ old('color', $category->color) == 'warning'   ? 'selected' : '' }}>{{ __('common.color_yellow') }}</option>
                                    <option value="danger"    {{ old('color', $category->color) == 'danger'    ? 'selected' : '' }}>{{ __('common.color_red') }}</option>
                                    <option value="secondary" {{ old('color', $category->color) == 'secondary' ? 'selected' : '' }}>{{ __('common.color_gray') }}</option>
                                </select>
