        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h2>{{ __('general.edit_work_item') }}: {{ $workItem->name }}</h2>
                </div>
                <div class="card-body">
                    <form action="{{ route('work.items.update', $workItem) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="3">{{ old('description', $workItem->description) }}</textarea>
                            @error('description')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
