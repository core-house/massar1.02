    ...
    window.addEventListener('load', function () {
        ...
        navigator.serviceWorker.register(window.RPOS_CONFIG.routes.serviceWorker, {
            ...
            function cacheSelfPage() {
                ...
                const cacheName = 'pos-v1.4.0-dynamic';
...
(function () {
    ...
    // ===== STATE =====
    let rCart = [];
    // expose للـ scripts الخارجية (scripts.blade.php)
    Object.defineProperty(window, 'rCart', { get: () => rCart, configurable: true });

    // ===== SERVER CONNECTIVITY CHECK =====
    // نتحقق من الاتصال بالسيرفر مباشرة (مش الإنترنت)
    // هذا يضمن الشغل على شبكة LAN بدون إنترنت
    let rIsOnline = false; // نبدأ بـ false حتى يثبت العكس
    let rPingTimer = null;
    ...
    async function checkServerConnectivity() {
        try {
            ...
            const res = await fetch(CFG.routes.ping, {
                method: 'GET',
                signal: ctrl.signal,
                cache: 'no-store',
                credentials: 'include',
    ...
    function updateOnlineUI(isOnline) {
        rIsOnline = isOnline;
        window.rposIsOnline = isOnline; // expose للـ scripts الخارجية
    ...
    async function pingAndUpdate() {
        const wasOnline = rIsOnline;
        const nowOnline = await checkServerConnectivity();
        updateOnlineUI(nowOnline);
        // لو رجع الاتصال، زامن كل الـ pending
    ...
    window.saveRPosInvoice = function (print = false) {
        ...
        const saveToServer = () => {
            $.ajax({
                ...
                headers: { 'X-CSRF-TOKEN': CFG.csrfToken },
                success: res => {
                    if (data.local_id && rDb) {
                        rDb.updateTransactionServerId(data.local_id, res.operhead_id).catch(() => {});
                    }
                    finalize(res);
                },
                error: () => {
                    updateOnlineUI(false);
                    rDb.saveTransaction(data)
                        .then(() => {
                            rposResetOrder();
                            showToast(CFG.i18n.saved_offline || 'تم الحفظ محلياً', 'info');
                        })
                        .catch(() => showToast(CFG.i18n.save_error, 'error'));
                },
                complete: resetSaving
            });
        };

        // ping السيرفر أولاً للتأكد من الاتصال الفعلي
        checkServerConnectivity().then(isConnected => {
            updateOnlineUI(isConnected);
            if (isConnected) {
                saveToServer();
            } else {
                rDb.saveTransaction(data)
    ...
    $(document).ready(function () {
        ...
        rDb.open().then(() => {
            ...
            // تحميل العملاء في IndexedDB لو متصل بالسيرفر
            if (rIsOnline) {
                syncAllPending();
                ...
                // ping أولي للتحقق من الاتصال بالسيرفر
                pingAndUpdate().then(() => {
                    if (rIsOnline) {
                        syncAllPending();
                        $.get(CFG.routes.searchCustomerPhone, { phone: '', load_all: 1 }, function (res) {
        ...
        // Online status — ping السيرفر مباشرة (يشتغل على LAN بدون إنترنت)
        const updateOnline = () => { pingAndUpdate(); };
        updateOnline();
        // ping كل 15 ثانية
        rPingTimer = setInterval(updateOnline, 5000);
        // أحداث المتصفح كـ hint سريع فقط
        ...
        function updatePendingBadge() {
            if (!rDb) return;
            rDb.getPendingTransactions().then(pending => {
                const count = (pending || []).filter(t => t.sync_status !== 'held' && !t.server_id).length;
