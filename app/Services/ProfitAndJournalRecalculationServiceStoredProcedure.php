...
class ProfitAndJournalRecalculationServiceStoredProcedure
{
    ...
    public function recalculateProfitForOperation(int $operationId): void
    {
        ...
        try {
            ...
            Log::info("Recalculated profit for operation {$operationId} using stored procedure");
        } catch (\Exception $e) {
            Log::error("Error in stored procedure for operation {$operationId}: " . $e->getMessage());
            throw $e;
        }
    }
    ...
    public function recalculateProfitsForAffectedOperations(array $itemIds, string $fromDate): void
    {
        ...
        try {
            ...
            Log::info("Recalculated profits for operations containing " . count($itemIds) . " items using stored procedure");
        } catch (\Exception $e) {
            Log::error("Error in batch stored procedure for profits: " . $e->getMessage());
            throw $e;
        }
    }
    ...
    public function recalculateAllAffectedOperations(array $itemIds, string $fromDate, ?int $currentInvoiceId = null, ?string $currentInvoiceCreatedAt = null): void
    {
        ...
        try {
            ...
            Log::info("Recalculated all affected operations for " . count($itemIds) . " items using stored procedures after date: {$fromDate}");
        } catch (\Exception $e) {
            Log::error("Error in stored procedure for all operations: " . $e->getMessage());
            throw $e;
        }
    }
}
