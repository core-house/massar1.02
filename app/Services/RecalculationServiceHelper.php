...
class RecalculationServiceHelper
{
    ...
    public static function recalculateAverageCost(array $itemIds, ?string $fromDate = null, bool $forceQueue = false, bool $isDelete = false): void
    {
        ...
        try {
            ...
            Log::info('Strategy selected for average cost recalculation', [
                'strategy' => $strategy,
                'item_count' => count($itemIds),
            ]);
    ...
    public static function recalculateProfitsAndJournals(array $itemIds, ?string $fromDate = null, ?int $currentInvoiceId = null, ?string $currentInvoiceCreatedAt = null): void
    {
        ...
        try {
            ...
            Log::info('Strategy selected for profits and journals recalculation', [
                'strategy' => $strategy,
                'item_count' => count($itemIds),
            ]);
    ...
    public static function recalculateManufacturingChain(array $rawMaterialItemIds, string $fromDate): void
    {
        ...
        try {
            ...
            Log::error('Invalid parameters for manufacturing chain recalculation', [
                'error' => $e->getMessage(),
        ...
        if (empty($rawMaterialItemIds)) {
            Log::info('No raw material IDs provided for manufacturing chain recalculation');
        ...
        if (! $chainEnabled) {
            Log::info('Manufacturing chain recalculation is disabled in configuration');
        ...
        try {
            ...
            if (empty($affectedInvoices)) {
                Log::info('No affected manufacturing invoices found', [
                    'raw_material_ids' => $rawMaterialItemIds,
                    'from_date' => $fromDate,
                ]);
            ...
            Log::info('Processing manufacturing chain recalculation', [
                'raw_material_ids' => $rawMaterialItemIds,
                'from_date' => $fromDate,
                'affected_invoices_count' => count($invoiceIds),
            ]);
            ...
            if (! empty($results['updated_items'])) {
                Log::info('Triggering average cost recalculation for affected products', [
                    'product_item_ids' => $results['updated_items'],
                    'count' => count($results['updated_items']),
                ]);
            ...
            Log::info('Manufacturing chain recalculation completed successfully', [
                'raw_material_ids' => $rawMaterialItemIds,
                'from_date' => $fromDate,
                'processed_invoices' => $results['processed_invoices'],
                'updated_items_count' => count($results['updated_items']),
            ]);
        } catch (\Exception $e) {
            $monitor->end($operationId, [
                'success' => false,
            ...
            Log::error('Manufacturing chain recalculation failed', [
                'error' => $e->getMessage(),
                'raw_material_ids' => $rawMaterialItemIds,
                'from_date' => $fromDate,
